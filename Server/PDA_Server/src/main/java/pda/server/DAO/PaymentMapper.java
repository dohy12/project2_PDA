...
public interface PaymentMapper {
	...
	@Insert("INSERT INTO ${GroupId}.payment_request VALUES (#{user_id}, '${merchant_uid}', '${buyer_name}', '${payment_name}', ${payment_price})")
	...
	@Select("SELECT merchant_uid, buyer_name, payment_name, payment_price FROM ${GroupId}.payment_request WHERE user_id=#{user_id}")
	public Map<String, Object> select_request_infos(@Param("GroupId") String GroupId, int user_id);
	
	//결제 요청 정보 삭제하기 (그룹 스키마, 결제 요청 테이블)
	@Delete("DELETE FROM ${GroupId}.payment_request WHERE user_id=#{user_id}")
	public void delete_request_infos(@Param("GroupId") String GroupId, int user_id);
	
	
	//결제 성공 시 결과 정보 저장하기 (그룹 스키마, 결제 결과 테이블)
	@Insert("INSERT INTO ${GroupId}.payment_result VALUES (#{user_id}, '${imp_uid}', '${merchant_uid}', '${time}')")
	public void insert_result_infos(@Param("GroupId") String GroupId, int user_id, @Param("imp_uid") String imp_uid, @Param("merchant_uid") String merchant_uid, @Param("time") String time);
	
	//결제 시간 정보 불러오기 (그룹 스키마, 결제 결과 테이블)
	@Select("SELECT time FROM ${GroupId}.payment_result WHERE user_id=#{user_id}")
	public List<String> select_result_time(@Param("GroupId") String GroupId, int user_id);
	
	//해당 시간의 결제 결과 정보 불러오기 (그룹 스키마, 결제 결과 테이블)
	@Select("SELECT imp_uid, merchant_uid FROM ${GroupId}.payment_result WHERE user_id=#{user_id} AND time='${time}'")
	public Map<String, Object> select_result_infos(@Param("GroupId") String GroupId, @Param("time") String time, int user_id);
